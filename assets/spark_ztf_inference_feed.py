...
def write_to_kafka(sdf, key, bootstrap_servers, sasl_username, sasl_password, topic_name, npart=10):
...
def check_path_exist(dateToCheck):
    r = requests.post(
        ...
        json={"date": "{}{}{}".format(*dateToCheck.split("-")), "columns": "basic:sci", "output-format": "json"},
    )
    return r.json() != []
...
def generate_spark_paths(startDate, stopDate, basePath):
    ...
    if startDate == stopDate:
        return [basePath + endPath.format(*startDate.split("-"))] if check_path_exist(startDate) else []
    paths = []
...
def add_fink_class(df):
    return df.withColumn(
        ...
        extract_fink_classification(
            df["cdsxmatch"], df["roid"], df["mulens"],
            df["snn_snia_vs_nonia"], df["snn_sn_vs_all"], df["rf_snia_vs_nonia"],
            df["candidate.ndethist"], df["candidate.drb"], df["candidate.classtar"],
            df["candidate.jd"], df["candidate.jdstarthist"],
            df["rf_kn_vs_nonkn"], df["tracklet"],
        ),
    )
...
def main(args):
    ...
    log.info("Generating data paths from {} to {}".format(args.startDate, args.stopDate))
    ...
    if args.fclass:
        ...
        other_class = [c.replace("(SIMBAD) ", "") for c in args.fclass if c not in tns_class]
        conditions = []
        if other_class:
