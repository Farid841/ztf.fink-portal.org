...
def _discover_component_image(model_name, version, component):
    ...
    try:
        ...
        if r.status_code != 200:
            logging.warning("[MLflow] model-versions/get %s@%s → HTTP %s", model_name, version, r.status_code)
            return None
        mv = r.json().get("model_version", {})
        tags = {t["key"]: t["value"] for t in mv.get("tags", [])}
        image = tags.get(tag_key)
        if not image:
            logging.warning("[MLflow] Tag '%s' missing for %s@%s", tag_key, model_name, version)
        return image
    except Exception:
        logging.warning("[MLflow] Could not get image tag for %s@%s\n%s", model_name, version, traceback.format_exc())
        return None


def create_k8s_inference_jobs(input_topic, output_topic, job_id, selected_models, inf_config):
    ...
    kafka_creds = [
        {"name": "KAFKA_BOOTSTRAP_SERVERS", "value": dt_config["KAFKA_BOOTSTRAP_SERVERS"]},
        {"name": "KAFKA_SECURITY_PROTOCOL", "value": protocol},
    ]
    if protocol not in ("PLAINTEXT", "SSL"):
        kafka_creds += [
            {"name": "KAFKA_SASL_USERNAME", "value": dt_config.get("KAFKA_SASL_USERNAME", "")},
            {"name": "KAFKA_SASL_PASSWORD", "value": dt_config.get("KAFKA_SASL_PASSWORD", "")},
            {"name": "KAFKA_SASL_MECHANISM", "value": dt_config.get("KAFKA_SASL_MECHANISM", "PLAIN")},
        ]
    ...
    for model_str in selected_models:
        ...
        model_safe = model_str.replace("@", "-").replace(".", "-").replace("_", "-").lower()
                    ...
                    "spec": {
                        ...
                        "containers": [{
                            ...
                            "env": kafka_creds + [
                                ...
                                {"name": "SCHEMA_TOPIC", "value": f"{input_topic}_schema"},
                                {"name": "OUTPUT_FORMAT", "value": "json"},
                                {"name": "SKIP_CUTOUTS", "value": "true"},
                                {"name": "AUTO_OFFSET_RESET", "value": "earliest"},
                                {"name": "CONSUMER_GROUP_ID", "value": f"fink-ai-pre-{model_safe}-{job_id}"},
        ...
        model_job = {
            "apiVersion": "batch/v1",
            "kind": "Job",
            "metadata": {"name": f"inf-model-{model_safe}-{ts}", "namespace": namespace},
            "spec": {
                "ttlSecondsAfterFinished": 3600,
                "parallelism": parallelism,
                "completions": parallelism,
                    ...
                    "spec": {
                        ...
                        "containers": [{
                            ...
                            "env": kafka_creds + [
                                ...
                                {"name": "OUTPUT_FORMAT", "value": "json"},
                                {"name": "AUTO_OFFSET_RESET", "value": "earliest"},
                                {"name": "CONSUMER_GROUP_ID", "value": f"fink-ai-model-{model_safe}-{job_id}"},
        ...
        try:
            ...
            logging.warning("[K8s] Job creation failed for %s\n%s", model_str, traceback.format_exc())
