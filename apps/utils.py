...
def _fetch_model_aliases(mlflow_uri, auth, model_names):
    ...
    for name in model_names:
        try:
            ...
            if r.status_code != 200:
                logging.warning("[MLflow] registered-models/get %s → HTTP %s", name, r.status_code)
                continue
        except Exception:
            logging.warning("[MLflow] Could not fetch aliases for %s\n%s", name, traceback.format_exc())
            continue
...
def get_available_models():
    ...
    try:
        ...
        if r.status_code != 200:
            logging.warning("[MLflow] model-versions/search → HTTP %s\n%s", r.status_code, r.text)
            return []
        versions = r.json().get("model_versions", [])
    except Exception:
        logging.warning("[MLflow] Could not reach %s\n%s", mlflow_uri, traceback.format_exc())
        return []

    ready_versions = [
        v
        for v in versions
        if {"preprocessing_image", "model_image"} <= {t["key"] for t in v.get("tags", [])}
    ]
